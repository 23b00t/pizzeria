...
require __DIR__ . '/../head.php'; ?>
  ...
  <?= htmlspecialchars($pizza->name()) ?></a></td>
  ...
  <?php if (!User::isAdmin()) : ?>
  <!-- purchase form -->
  <form action="./index.php?area=purchase&action=handle" 
  method="POST" style="display:inline;">
  <input type="hidden" name="pizza_id" value="<?= htmlspecialchars($pizza->id()) ?>">
  <input type="number" name="quantity" min="1" value="1" 
  class="form-control-sm" style="width: 80px; display:inline-block;" required>
  <!-- insert csrf_token -->
