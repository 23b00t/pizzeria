...
// @var app\controllers\PizzaController $pizza
// @var app\controllers\PizzaController $ingredients
...
<div class="container mt-5">
    ...
    <form action="<?= isset($pizza) ? './index.php?area=pizza&action=update&id='
        . htmlspecialchars($pizza->id()) : './index.php?area=pizza&action=store' ?>" method="POST">
        <div class="form-group mb-3">
            <label for="name">Pizza Name</label>
            <input type="text" name="name" id="name" class="form-control" 
                value="<?= isset($pizza) ? htmlspecialchars($pizza->name()) : '' ?>" required>
        </div>
        
        <div class="form-group mb-3">
            <label for="price">Preis (€)</label>
            <input type="number" step="0.01" name="price" id="price" class="form-control" 
                value="<?= isset($pizza) ? htmlspecialchars($pizza->price()) : '' ?>" required>
        </div>
        ...
        <div class="form-group mb-3">
            ...
            <?php foreach ($ingredients as $ingredient) :
                ...
                <input type="number" name="quantities[<?= $ingredient->id() ?>]" class="form-control mt-1" 
                    placeholder="Stückzahl" min="0" value="<?= htmlspecialchars($quantity) ?>">
                </div>
            <?php endforeach; ?>
        </div>
        ...
        <button type="submit" class="btn btn-primary">
            <?= isset($pizza) ? 'Pizza aktualisieren' : 'Pizza erstellen' ?>
        </button>
        <a href="./index.php?area=pizza&action=index" class="btn btn-secondary">Abbrechen</a>
    </form>
</div>
